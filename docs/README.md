# Documentation

Cette section regroupe la documentation du projet. Les pages ci-dessous sont indépendantes et peuvent être lues dans n’importe quel ordre.

## Table des matières

- [Architecture](00-Architecture.md)
- [Setup](10-Setup.md)
- [Ingestion](20-Ingestion.md)
- [dbt — Modélisation](30-dbt-Modeling.md)
- [dbt Docs & Lineage](31-dbt-Docs-Lineage.md)
- [Dashboard Streamlit](40-Dashboard.md)
- [Orchestration Prefect](50-Orchestration-Prefect.md)
- [CI/CD (GitHub Actions)](60-CI-CD.md)
- [Annexe — Inspection DuckDB](90-Appendix-Warehouse-Inspection.md)
